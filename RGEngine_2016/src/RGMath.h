...
#define RG_M_DEG_PER_RAD 57.2957795130823069f
		...
		bool unequal(const double &a, const double &b);
		bool unequal(double &&a, double &&b);
		bool unequal(const float &a, const float &b);
		bool unequal(float &&a, float &&b);
		...
		struct Vector
		{
			...
			bool Intersected(const Vector &v) const;
		...
		struct Circle
		{
			...
			float Circumference(void) const; // 둘레
		...
		struct Line
		{
			...
			void SetLine(const Vector &p1, const Vector &p2);
